...
describe("PWA precache manifest (built dist/)", () => {
  ...
  it("precaches the self-hosted font files, so display/UI type doesn't silently fall back offline", () => {
    // Fraunces/Schibsted Grotesk are self-hosted (issue #136, #246) specifically
    // so the PWA never depends on a network fetch to a font CDN. That guarantee
    // only holds if the woff2 files actually end up in the precache manifest.
