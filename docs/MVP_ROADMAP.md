...
- **Day 3** — Build the automated dedup/schema validator (#15) against fixtures, before any recipe-template batch is generated — validating only after all batches land would catch cross-batch id collisions and schema drift once 170 templates already exist, not as each batch is produced.
- **Day 3-5** — AI-generate recipe templates in small batches against the coverage matrix (never one big "generate 200 recipes" prompt — that produces repetitive, generic output), explicitly feeding back already-generated dish names each batch to force diversity, running the validator against each batch as it lands.
- **Day 5-6** — Spot-check a stratified ~15-20% sample for tone and quality; hand-polish a small **"hero" subset of ~20-30 templates** that will disproportionately appear in early "Tonight" suggestions — this is where product voice and first-impression quality matter most.
