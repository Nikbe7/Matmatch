...
2. **Pick a main ingredient** — either choose one directly (e.g., "chicken") or tap "suggest for me" (AI/engine suggests based on season, price tier, and household history). Shipped as a tap grid of the proteins the most dinner templates are built from, derived from the catalog rather than hand-picked, plus *Föreslå åt mig* — which reads the main ingredient off the best-ranked candidate, so season, cost tier and history decide it through the existing score rather than a second opinion. No search box, no text input, here or anywhere else in this flow.
3. **Quick pantry input** — a multi-select grid of common ingredients (not a search box), e.g. tapping "rice," "cream," "onion" — this is intentionally ephemeral, per-session input, not a persistent inventory (see Architecture doc, item 4)
...
- **No good direction fits pantry input**: offer to loosen constraints (e.g., "add 1-2 more ingredients to unlock better options") rather than a dead end.
