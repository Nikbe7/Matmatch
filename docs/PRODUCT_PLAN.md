...
| Dietary handling treated as a soft preference | A wrong suggestion breaks a promise the product made | Hard deterministic filtering in the Meal Engine, never dependent on model output |
| Making a safety promise the data cannot back | An allergy filter is only as good as the mappings behind it, and those failed to stay maintained at catalog scale | Allergy filtering was withdrawn rather than half-kept (#224, DECISION_LOG 2026-08-25); reinstating it starts with who maintains the mappings, not with the code |
