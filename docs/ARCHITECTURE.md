...
**Separate deterministic logic from generative AI, and only let AI touch the parts of the problem that genuinely need creativity or natural language.** Ingredient matching, cost tiering, seasonality, portion math, and dietary filtering are all well-defined enough to be regular application logic — fast, free, deterministic, and testable. AI is reserved for: proposing creative meal directions, adapting/personalizing a suggestion, and handling free-text refinement requests. This is both a cost-control strategy and a correctness strategy (dietary filtering must never depend on model behavior). Allergy filtering was part of this list until #224 removed it from the product entirely — see §4.3.
...
### 4.3 Hard constraints stay out of AI entirely
Hard dietary restriction filtering happens in the **Meal Engine**, before any candidate reaches the AI layer or the user's screen. The AI Orchestrator is never the last line of defense for a hard constraint — it operates only within a pre-filtered candidate set. Since #112 the filter is scoped to the members eating a given meal rather than to the household's union; that scoping is itself deterministic and fail-closed — any diner set that is absent, empty or not wholly valid resolves to the whole household (DECISION_LOG 2026-08-09).

**Allergy filtering is gone (#224, DECISION_LOG 2026-08-25).** The app no longer asks about allergies, stores no answer it acts on, and makes no claim about allergens. What failed was not this design but the maintenance behind it: the mappings required 100% manual verification with no sampling, which was feasible at 206 ingredients and is not at the thousands the catalog needs. A curated allergen database nobody can maintain is not a safety feature — the promise outlives its coverage, which is worse than no promise. Do not reinstate the filter without first answering who maintains the mappings at that scale, and never let a model fill them in while anything filters on them.

Two things deliberately survived and must not be confused with the removed gate. `isIngredientUnknown` (`src/engine/catalog.ts`) was the gate's *other* fail-closed condition and was never an allergy rule — "we cannot say what an unknown ingredient even is" — so a template or substitution group naming an id the catalog lacks is still withheld, for everyone. And `dietary_flags` are untouched: a wrong answer there is an irritated user, not an emergency room, and they only ever shared a module with allergies by historical accident.

## 5. Database Design (Conceptual Schema)

- **User** — id, email, auth provider info, subscription status
- **Household** — id, owner_user_id, preference_price / preference_time / preference_variation / preference_simplicity (0–100 in steps of 5, all defaulting to 0). The ownership anchor plus the household's **persistent preference baseline** (#157, DECISION_LOG 2026-08-16): the four axes the "Vad är viktigt för er?" sliders edit. Ranking-only — `selectCandidateTemplates` is never handed a weight vector, so no slider position can widen dietary filtering. `src/schema/preferenceWeights.ts` is the single axis definition, shared by this baseline and by the session-scoped chip delta, and `toRankingWeights` is the only bridge into engine units; all-zero reproduces the pre-#157 constants exactly, which is what makes the migration behaviour-preserving. `preference_simplicity` is stored but **inert and unrendered** until #151 supplies a curated effort signal. It holds no constraint fields of its own. A household's effective dietary set is **derived** from its members, never stored, so there is one source of truth for it. `mealDiners` (`src/engine/constraints.ts`) is the single entry point every request uses: it resolves the diner set once and derives the constraint set and the portion total from that one resolution, so the two can never describe different people (#112)
- **HouseholdMember** — id, household_id, type (adult/child), portion_factor, name (optional), dietary_flags[] (**per-member**; see "Dietary vocabulary" below for the locked value list). Constraints were household-level until #115 reversed that — a household does not hold a restriction, people do, and only the per-member shape can answer *whose* a given restriction is, which is what scoping a meal to the people eating it requires (DECISION_LOG 2026-08-09). The `allergies[]` column still exists in the database, unused and unread since #224, so the removal reverts with `git revert` rather than a down migration; dropping it is its own later slice
...
- **RecipeTemplate** — id, name, protein_group, cuisine, cost_tier, prep_time_band, dietary_tags[], ingredient_slots[] (role, ingredient_id, substitutable, quantity) — see "RecipeTemplate schema & coverage matrix" below for the locked vocabularies and matrix
- **IngredientAllergenMapping** — ingredient_id (FK to Ingredient.id), allergens[], verification_status — a preserved, hand-verified record that nothing reads since #224; see "Ingredient-to-allergen mapping" below
...
- **AnalyticsEvent** (`analytics_events`, built — issue #91) — id, household_id, event_name, payload (jsonb), client_timestamp, server_timestamp, seq. The destination for the typed events `web/src/analytics.ts` emits (`refinement_chip_tap`, `refinement_session_abandoned`, `meal_chosen`, `meal_choice_history_failed`, `app_error_shown`, `main_search_miss`) — Phase 1's success metrics (MVP_ROADMAP.md: Weekly Active Deciders, zero-input acceptance rate, repeat-use rate, Tier 0 vs 1/2 ratio) are counted from these. `event_name` and `payload` are validated against a server-side zod mirror of the frontend union (`src/api/routes/analytics.ts`) before any row is written — an unrecognised event name is a 400, not a stored row. Same shape as `cooked_meals`: append-only (SELECT/INSERT only, no UPDATE/DELETE), RLS scoped to the owning household through household_id. No PII beyond household_id — payloads carry only the fields the frontend union already declares (chip ids, weights, template ids, reroll depth), plus the one deliberate exception: `main_search_miss` carries the typed query, trimmed and capped at 64 characters server-side (#255). Read in order as `order by seq` (`readHouseholdEvents`) and never by `server_timestamp` — `now()` is transaction start time, so a whole batch shares one value and two overlapping flushes carry them inverted; `seq` is the write order, and `unique (household_id, seq desc)` is what makes it total rather than left to the plan (#98).
...
- The app collects no allergy data at all since #224 — one fewer category of sensitive personal data to hold (see Section 7).
...
### 5.2 Dietary vocabulary (locked, Phase 0 Day 2-3)

**Declared per household member, not per household** (#115) — see §5 above. What the Meal Engine filters against is the union over whichever members a given meal is for, which defaults to all of them.

**No household member declares an allergy any more (#224).** The `ALLERGIES` list — `gluten`, `dairy_lactose`, `egg`, `tree_nuts`, `peanuts`, `shellfish`, `fish`, `soy` — survives in `src/schema/vocabulary.ts` for exactly one reason: it is the value vocabulary of the preserved `data/ingredient-allergens.json` (§5.4), which is still schema-validated and read by nothing. It is not a household-facing vocabulary and nothing filters on it.
...
**Not included in MVP dietary flags, documented as future considerations:** pescatarian; gluten-free as a lifestyle choice (there is no way to express avoiding gluten at all since #224 removed the `gluten` allergy along with the rest, so this is now the *only* route to it and no longer has a stricter alternative to be compared against); religious dietary restrictions (halal, kosher, no-pork) — not covered by the current MVP use case, but plausible for the Swedish market and would need a decision on whether they behave as hard filters or soft preferences before being added; low-carb/keto and other diet systems, out of scope unless a real usage signal supports them.
...
- `familiarity` (added Phase 1, #72) — `everyday` | `occasional` | `adventurous`, required, no default. **Authored, not derived**, same rationale as `meal_types`. Unlike `meal_types`, this is a **ranking signal only, never a filter** — it feeds a penalty term in `scoreCandidate` (`src/engine/ranking.ts`) so Tonight favors ordinary weeknight food by default, but an `adventurous` template stays fully selectable. The *dish's* familiarity stays authored data, but since #157 the household controls how hard it is penalised: the Variation axis scales the familiarity step weight from its neutral 1.5 down to 0, so "Vi lyfter fram rätter ni inte lagat förut" genuinely removes the novelty penalty rather than merely softening it. This supersedes the 2026-07-31 position that familiarity is excluded from the weight vector, alongside that entry's rejection of sliders (DECISION_LOG 2026-08-16 and 2026-08-19). See DECISION_LOG for the classification pass and how the penalty weight is calibrated against `NEUTRAL_PREFERENCE_WEIGHTS`.
- `ingredient_slots[]` — each slot is `{role, ingredient_id, substitutable, quantity}`. `role` is the CLAUDE.md-specified subset of `Ingredient.category` used for slot composition: `protein`, `starch`, `vegetable`, `aromatic` (maps to `spice_aromatic`), `dairy`. `substitutable` gates whether the slot accepts swaps at all; the swap candidates themselves live in the substitution groups of §5.5, not on the template.
- `quantity` (added Phase 1, #123) — how much this slot contributes, authored for **4 portions** (`REFERENCE_PORTIONS`, `src/engine/quantities.ts`), required with no default. Either `{kind: "amount", amount, unit}` in the closed 8-unit vocabulary (`g`, `dl`, `msk`, `tsk`, `krm`, `st`, `klyfta`, `kruka`) or the explicit `{kind: "to_taste"}` marker; the validator rejects a slot carrying neither, so a missing amount fails rather than defaulting. **Curated, never generated at request time** — a quantity is a number the household acts on in a shop, so it follows the same offline drafted-and-spot-checked posture as cost tiers and seasonality (DECISION_LOG 2026-08-10/-11). Scaling to the diners eating tonight is deterministic and lives in exactly one place (`scaleSlotQuantity`), which also owns the per-unit rounding rule that keeps a scaled amount buyable. The quantity belongs to the **slot**, not to the ingredient in it: a substituted slot keeps its own amount, since the swap changes what fills the hole in the dish, not how big the hole is.

**Allergens are deliberately *not* a RecipeTemplate field.** No `contains_allergens[]` or similar is stored on the template, and #224 is not a reason to add one — it is the opposite. A hand-tagged allergen list on a template would be a claim about a dish that goes stale the moment a swap changes what is in it, maintained by nobody, on a path where nothing filters. `dietary_tags[]` is different and stays: it is authored per generation batch about the dish as a whole, and it is what the hard dietary filter reads.
...
### 5.4 Ingredient-to-allergen mapping (locked Phase 0 Day 3; unread since #224)

**Read by nothing since #224.** This was the single source of truth for allergen filtering (§4.3); that filter is gone, `loadEngineData` no longer loads the file, and no code path can consult an allergen. The 206 hand-verified rows are kept rather than deleted — real work, and a record worth having if the question is ever reopened — and `npm run validate` still checks them so the record cannot silently rot. The rest of this section describes the artifact as it stands; it is not a description of live behaviour.
...
**The fail-safe enforcement rule this section used to state — missing row and `unverified` row both treated as containing the allergen — described a filter that no longer exists.** It is recorded here as history, not as a requirement on anything running: nothing reads the file, so there is no permissive default left to guard against. It becomes binding again only if a filter is ever reinstated, and reinstating one is gated on §4.3's maintenance question, not on this rule.

Storage: `data/ingredient-allergens.json`, alongside `data/ingredients.json` / `data/recipe-templates.json` / `data/substitutions.json`. Validated via the CLI validator (#15) as `--type ingredient-allergen`: schema, duplicate ids, and referential integrity (every row points at a real catalog ingredient), plus an unverified-row count in the summary. **The coverage check in the other direction — every catalog ingredient has a mapping row — was removed in #224.** It enforced the 100% coverage the old fail-safe posture needed, and keeping it would block the very thing #224 exists to unblock: the first ingredient added to a growing catalog would fail validation for lacking a row nothing reads. Rows must point at real ingredients; ingredients no longer have to point back.
...
**Groups must never encode allergen or dietary suitability — in any form.** No allergen field, no dietary flags, no "dairy-free alternatives" group whose *identity* is the restriction it satisfies. A suitability claim baked into a group is a hand-curated promise about a food, maintained nowhere, exactly the kind of claim #224 removed from the product — and for dietary suitability, which the engine does still filter on, it would be a second source of truth competing with the template's own `dietary_tags`. What a group member *is* checked for at swap time is only that the catalog knows it (`isIngredientUnknown`, §4.3) — a group may legitimately name an id the catalog lacks, and an unresolvable member is never offered.
...
- Sweden/EU context means **GDPR applies directly**. Household member data (names, member types, children's dietary information) is personal data deserving explicit handling: clear consent at collection, minimal retention, and easy deletion/export.
- **#224 removed a whole category of it.** The app no longer asks for or acts on allergies — health-adjacent data it was collecting and is not any more. The `household_members.allergies` column remains, unwritten with any value and unread, until its own removal slice; until then it holds only empty lists, and it should not be repurposed.
