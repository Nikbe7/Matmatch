...
>
> **`anon`, `authenticated` and `service_role` get nothing automatically either — but only since #267.** Supabase's bootstrap *did* carry `ALTER DEFAULT PRIVILEGES` for those three, so until `20260913120000_revoke_client_role_privileges.sql` every new table arrived granting them TRUNCATE, REFERENCES, TRIGGER and MAINTAIN, and every new sequence granting them UPDATE (`setval`). **TRUNCATE is not subject to RLS** — no policy can refuse it — so the append-only guarantee on `analytics_events` and `cooked_meals` was scoped to DELETE and did not cover it. Demonstrated before the fix: a signed-in user who could `select` zero rows and whose `delete` was refused could still `truncate` the table and erase every household's history. That migration revokes the defaults as well as the standing grants, so a new table now arrives with no client-role privileges at all and needs an explicit grant if a client role should reach it. `src/db/rls.test.ts` asserts this against the live catalog, including for a table created during the test — so a regression fails the suite rather than waiting for a reader to notice.
